...
- [ ] **TASK-9: Resolve ambiguous "I am on the clients page" step**
      `FeatureContext::iAmOnTheClientsPage()` (`@Given I am on the clients page`) collides with the generic `iAmOnThePage(:path)` (`@Given I am on the :path page`), which maps `clients` → `/clients`. Behat aborts the 3 Clients scenarios with an ambiguous-match error. Fix: remove the redundant specific `iAmOnTheClientsPage()` method (the generic step already handles it). Verify all 3 Clients scenarios pass their defined steps.
