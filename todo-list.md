...
- [x] **Fix `features/Payments/AdvancedPayments.feature:19` — undefined late fee steps**
      Scenario "User can apply late fees" now passes. Changes: added "Add Late Fee" button (JS modal opener, `type="button"`) to invoice show view (visible only when `$invoice->is_overdue`); added late fee modal with `late_fee_amount` input and "Apply" submit button; added `invoices.addLateFee` route; added `InvoiceController::addLateFee` method that creates a "Late Fee" line item and calls `recalculateTotals()`; defined step defs `iEnterLateFeeAmount` (fills `late_fee_amount` field, adds to `lastFilledFields` so `iPress` can find the form) and `theInvoiceTotalShouldIncreaseBy` (verifies the late fee line item exists with the expected amount).

- [x] **Fix `features/Projects/Projects.feature:19` — undefined time entry form steps**
      Scenario "User can create a time entry" now passes. Changes: fixed `pageMap` URLs for time entries (`/projects/time-entries` → `/time-entries`); changed index page link text from "+ New Entry" to "Add Time Entry"; changed create form submit button from "Create Time Entry" to "Save"; made `start_time` nullable in `TimeEntryController::store` with a default of `now()` (manual hours entry doesn't need start/end times); defined `iFillInTheTimeEntryFormWith` (maps project name → `project_id`, fills hours/description) and `theHoursShouldBeRecorded` (asserts the latest TimeEntry has non-null hours > 0).
