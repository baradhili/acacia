...
- [x] Look at how to make things modular using nwidart - look at https://github.com/akaunting/akaunting - for plugins from somewhere - maybe just github first off? - (Sep 2026, branch feat/modular) Done; plan in .zcode/plans/modularisation.md, docs in docs/modules.md. nwidart/laravel-modules ^13 installed (merge-plugin wired in root composer.json so Modules/*/composer.json autoload); the shell exposes akaunting-style registration contracts — a nav registry replaced the 58 hardcoded sidebar/topbar links and the dashboard grid renders a widget registry — so modules contribute UI without touching core views. Three modules extracted with history preserved (Payroll+PSI, Reconciliation, Shares/franking/dividends; all core:true, only disableable), with core seams degrading via class_exists (BAS W1/W2, the settlement franking hook, tax-report balances). Plugin management is akaunting's GUI flow minus the store: the admin Modules screen enables/disables, updates git-installed modules, uninstalls (migrations rolled back, dir deleted; refused for core), and installs from a GitHub URL via clone→validate→move→migrate→enable with nothing left behind on failure (ModulesAdminTest installs a local git fixture hermetically). modules_statuses.json ships committed so fresh clones boot modules enabled. Remaining: Estimates and BAS/tax modules (BAS gated on decomposing ReportController), and pushing the verified payroll subtree split (split/erp-payroll branch) to its own repo. Suite 856.
