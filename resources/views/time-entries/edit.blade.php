...
@section('content')
        ...
        <form action="{{ route('time-entries.update', $timeEntry) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select name="project_id" id="project_id"
                        ...
                        @foreach($projects as $id => $name)
                            <option value="{{ $id }}" {{ old('project_id', $timeEntry->project_id) == $id ? 'selected' : '' }}>{{ $name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="purchase_order_id" id="purchase_order_id"
                        ...
                        @foreach($purchaseOrders as $id => $poNumber)
                            <option value="{{ $id }}" {{ old('purchase_order_id', $timeEntry->purchase_order_id) == $id ? 'selected' : '' }}>{{ $poNumber }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="entry_date" id="entry_date" value="{{ old('entry_date', $timeEntry->entry_date?->format('Y-m-d')) }}" required
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    @error('entry_date')
                ...
                <div>
                    ...
                    <input type="number" name="hours" id="hours" value="{{ old('hours', $timeEntry->hours) }}" step="0.01" min="0" max="24" required
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                ...
                <div>
                    ...
                    <input type="time" name="start_time" id="start_time" value="{{ old('start_time', $timeEntry->start_time?->format('H:i')) }}"
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                ...
                <div>
                    ...
                    <input type="time" name="end_time" id="end_time" value="{{ old('end_time', $timeEntry->end_time?->format('H:i')) }}"
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
            ...
            <div class="mt-6">
                ...
                <div id="breaksContainer" class="space-y-2">
                    @php
                        $oldBreaks = old('breaks', $timeEntry->breaks->map(fn ($b) => [
                            'start' => $b->start_display,
                            'end' => $b->end_display,
                        ])->all());
                    @endphp
            ...
            <div class="mt-6 flex items-center">
                <input type="checkbox" name="billable" id="billable" value="1" {{ old('billable', $timeEntry->billable ? '1' : '') ? 'checked' : '' }}
    ...
    @push('scripts')
        <script>
            let breakIndex = {{ max(count($oldBreaks ?? []), 0) }};
