...
@section('content')
    ...
    <div class="bg-white rounded-lg shadow p-6 max-w-4xl mb-6">
        <h2 class="text-lg font-semibold text-gray-800 mb-1">Unsettled GST position</h2>
        <p class="text-sm text-gray-500 mb-4">
            Everything never settled to date — the balances of the GST Payable and GST Receivable
            accounts, across any number of quarters (and even closed financial years), because a
            settlement is only ever recorded here. Claiming GST late simply means settling at a
            later date.
        </p>
        ...
        <dl class="grid grid-cols-3 gap-4 text-sm border-y border-gray-100 py-4">
            <div>
                <dt class="font-medium text-gray-700">GST payable (collected)</dt>
                <dd class="text-lg text-gray-800">${{ number_format($position['payable'], 2) }}</dd>
            </div>
            <div>
                <dt class="font-medium text-gray-700">GST receivable (credits)</dt>
                <dd class="text-lg text-gray-800">${{ number_format($position['receivable'], 2) }}</dd>
            </div>
            <div>
                <dt class="font-medium text-gray-700">Net</dt>
                <dd class="text-lg font-bold {{ $position['net'] >= 0 ? 'text-red-700' : 'text-green-700' }}">
    ...
    <div class="bg-white rounded-lg shadow p-6 max-w-4xl mb-6">
        ...
        <p class="text-sm text-gray-500 mb-4">
            Posts one journal that nets the two GST accounts and moves the difference to/from the
            bank: <code>Dr GST Payable / Cr GST Receivable / Cr Bank</code> when paying the ATO,
            mirrored for a refund. A BAS is lodged and paid in the month after its quarter, so the
            bank date normally lags the period covered.
        </p>

        <form method="POST" action="{{ route('bas-settlements.store') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            @csrf
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="text-left text-xs text-gray-500 uppercase tracking-wider">
                        <th class="py-2 pr-4">Covers GST to</th>
