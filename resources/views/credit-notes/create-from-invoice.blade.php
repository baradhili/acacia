@extends('layouts.app')
@section('title', 'Create Credit Note from ' . $invoice->invoice_number)
@section('content')

    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-800">Create Credit Note</h1>
        <p class="text-sm text-gray-500 mt-1">
            Crediting against invoice <span class="font-semibold">{{ $invoice->invoice_number }}</span>
    ...
    <form action="{{ route('credit-notes.store') }}" method="POST" class="space-y-6">
        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <p class="text-sm text-gray-500 mb-4">Pre-filled from the invoice — adjust quantities or prices to credit a partial amount.</p>

            <div id="itemsContainer">
                @php $items = old('items', $creditNoteItems ?: [[]]); @endphp
    ...
    <script>
        let itemIndex = {{ count(old('items', $creditNoteItems ?: [[]])) }};
