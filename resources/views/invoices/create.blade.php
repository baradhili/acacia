...
@section('content')
    ...
    <form action="{{ route('invoices.store') }}" method="POST" class="space-y-6">
                    ...
                    <div class="space-y-2">
                        @foreach($timeEntries as $entry)
                            <label class="flex items-start p-2 bg-white rounded">
                                ...
                                <div class="ml-3">
                                    <p class="text-sm font-medium text-gray-900">{{ $entry->description ?? 'No description' }}</p>
        ...
        <div class="flex justify-end gap-4">
            <a href="{{ route('invoices.index') }}" class="bg-gray-100 hover:bg-gray-200 text-gray-700 px-6 py-2 rounded-lg">
                Cancel
            </a>
