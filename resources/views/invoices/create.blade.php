            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <select name="purchase_order_id" id="poSelect"
                        ...
                        @php
                            $allPOs = App\Models\PurchaseOrder::with('client')
                                ->open()
                                ->orderBy('po_number', 'desc')
                                ->get();
                        @endphp
                        @foreach ($allPOs as $po)
                            <option value="{{ $po->id }}" data-client-id="{{ $po->client_id }}"
                                {{ old('purchase_order_id', $selectedPO?->id) == $po->id ? 'selected' : '' }}>
                                {{ $po->po_number }} - {{ Str::limit($po->title, 25) }}
                                ({{ $po->client->name ?? 'No client' }})
                            </option>
                        @endforeach
                    </select>
                </div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            @if ($timeEntries->isNotEmpty())
                <div class="mb-4 p-4 bg-blue-50 rounded-lg">
                    <h3 class="font-medium text-blue-800 mb-2">Unbilled Time Entries</h3>
                    <div class="space-y-2">
                        @foreach ($timeEntries as $entry)
                            <label class="flex items-start p-2 bg-white rounded">
                                ...
                                <div class="ml-3">
                                    ...
                                    <p class="text-xs text-gray-500">
                                        {{ $entry->hours }} hrs @ ${{ number_format($entry->effective_rate, 2) }}/hr
                                        = ${{ number_format($entry->hours * $entry->effective_rate, 2) }}
                                    </p>
                                </div>
                            </label>
                        @endforeach
                    ...
                    <div class="item-row grid grid-cols-12 gap-2 mb-4 p-4 bg-gray-50 rounded-lg">
                        ...
                        <div class="col-span-1">
                            ...
                            <input type="number" name="items[{{ $index }}][discount_percent]"
                                value="{{ $item['discount_percent'] ?? 0 }}" step="0.01" min="0"
                                max="100"
                                class="rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 w-full text-sm">
                        </div>
                        <div class="col-span-2">
