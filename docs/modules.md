...
- **Nav** — `App\Support\Nav`: `addSidebar(items)`, `addTopbar(items)` and
  `addTopbarChild('Setup', $item)` to slot one item into a core dropdown
  (the child's active-route patterns join the dropdown's). Items carry
  `type` (link/heading/divider/dropdown), `position`, `roles`, `active`
  route patterns, `icon` (inner SVG) and optional `add` shortcut routes.
